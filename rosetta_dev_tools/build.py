...
you don't want to use whichever CMake picks by default, set these variables and 
rebuild rosetta from scratch.
...
Options:
    -u, --update
        Update the CMake configuration files to account for source files that 
        have been added or removed (or more specifically, to account for 
        changes to the *.settings files in `source/src`).
...
def build_rosetta(build=None, project=None, update=False, clean=False,
        rosetta_path=None, verbose=False):
    ...
    rosetta_path = rosetta_path or helpers.find_rosetta_installation()
    cmake_path = os.path.join(rosetta_path, 'source', 'cmake')
    build_path = os.path.join(cmake_path, 'build_' + build)
    ninja_config = os.path.join(build_path, 'build.ninja')
    ...
    # '--update' flag was passed as a command line option.

    make_project = 'python2', 'make_project.py', 'all'
    make_ninja = 'cmake', '-G', 'Ninja', '-Wno-dev'

    if update or not os.path.exists(ninja_config):
    ...
    if project:
        ninja_build += project, #project + '_symlink'
